...
- **Pixel-Perfect 像素级描边轮廓**：
    - 蓝色实线 (选中) / 白色虚线 (悬停) 均采用贴图采样生成，支持旋转和缩放
    - **性能缓存**：描边贴图自动缓存，避免重复生成开销

### 6. 极速动画性能优化 (v1.1.0+)

- **订阅式渲染 (Subscription Mode)**：
    - 播放动画时，帧号更新通过 `EventTarget` 广播，绕过 React 的 `render` 循环。
    - 只有当用户 **手动切帧** 或 **停止播放** 时，才会同步回 React State。
- **Zero-DOM 刻度线**：
    - 时间轴网格线使用 CSS `linear-gradient` 实现，万级刻度瞬时加载，零内存占用。
- **60FPS 保证**：
    - 消除 React Reconciliation 耗时，PIXI 渲染平均耗时 < 0.5ms，支持复杂骨架流畅播放。
    - **悬停**：白色虚线矩形 + **白色半透明像素级像素轮廓**
    - **选中**：蓝色实线矩形 + **蓝色半透明像素级像素轮廓**，并清除悬停状态
    - 轮廓基于贴图 alpha 通道实时生成，位置精确对齐图片边缘，支持旋转、缩放和骨骼变换
    - **非阻塞交互**：高亮层设定为 `eventMode = 'none'`，确保图片选中操作灵敏顺畅
